...
import org.example.backend.mapreduce.commentsum.CommentjobsumMapper;
import org.example.backend.mapreduce.commentsum.CommentjobsumReducer;

public class Undertwojob {
    public void run(String kv) throws Exception {
        ...
        job.setJarByClass(Undertwojob.class);
        String jobname = "Undertwojob"+kv;
        job.setJobName(jobname);
        String url2 = "/dataanalyse/crawler/MergePublish/book/";
        String url3 =  kv + "total.txt";
        Path input = new Path(url2+url3);
        FileInputFormat.addInputPath(job,input);
        String urx = "undertwojob"+kv;
        job.setMapperClass(UndertwoMapper.class);
        job.setMapOutputKeyClass(Text.class);
        job.setMapOutputValueClass(FlowBean.class);
        TableMapReduceUtil.initTableReducerJob(urx, UndertwoReduce.class, job);
