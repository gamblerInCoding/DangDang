...
ways=wayj[:-len("\src\main\java\org\example\backend\machinelearn\way.py")]
for i in range(1,9):
...
uls = ways+"book"+str(10)+".xls"
dfs = pd.read_excel(uls,header=None,names=["ranking","imag","name","comment","recommond","author","publish","fivestars","price"])
hah=dfs['name']
del(dfs['imag'])
del(dfs['name'])
del(dfs['fivestars'])
...
result.to_excel(excel_writer="result1.xls",index=False,encoding='utf-8')
